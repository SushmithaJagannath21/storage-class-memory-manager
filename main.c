...
static int remove_node(struct avl *avl, const char *s)
{
	uint64_t count;
	if (!s || safe_strlen(s) == 0)
	{
		fprintf(stderr, "Error: Missing argument\n");
		return -1; /*Indicating an error due to missing string*/
	}

	count = avl_exists(avl, s);
	if (count == 0)
	{
		fprintf(stderr, "'%s' does not exist in the tree\n", s);
		return -1; /*Indicating that the item was not found*/
	}

	if (avl_remove(avl, s) == -1)
	{
		fprintf(stderr, "Error: Unable to remove '%s'\n", s);
		return -1; /* Indicating an error during removal */
	}
	else
	{
